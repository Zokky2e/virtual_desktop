// lib/core/providers/local/local_wallpaper_repository_io.dart
import 'dart:async';
import 'dart:convert';
import 'dart:io';
...
///
/// Reached only through local_wallpaper_repository.dart's conditional
/// export, so the `dart:io` import above never enters the web build.
class LocalWallpaperRepository implements WallpaperRepository {
  ...
  Future<void> _persist(List<WallpaperItem> items) async {
    final file = await _indexFile();
    await file.writeAsString(jsonEncode(items.map(_toJson).toList()));
  }

  void _emit(List<WallpaperItem> items, String ownerId) =>
    ...
    try {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (w.id == itemId && !w.isSet) {
          items[i] = _withIsSet(w, true);
        } else if (w.id != itemId && w.isSet) {
          items[i] = _withIsSet(w, false);
        }
      }
      await _persist(items);
      _emit(items, ownerId);
